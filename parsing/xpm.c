...
/*   Updated: 2025/01/31 18:54:50 by mokariou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	set_xpm(t_game *game, char *old_path, t_xpm *xpm, int i)
{
	...
	xpm[i].img_ptr = mlx_xpm_file_to_image(game->mlx, path, &xpm[i].width,
			&xpm[i].height);
	if (!xpm[i].img_ptr)
		return (error("failed xpm"), true);
	xpm[i].data = (int *)mlx_get_data_addr(xpm[i].img_ptr, &xpm[i].bbp,
			&xpm[i].size_line, &xpm[i].endian);
	if (!xpm[i].data)
		return (true);
	...
	while (++y, y < xpm[i].height)
	{
		...
		while (++x, x < xpm[i].width)
			xpm[i].colors[y * xpm[i].width + x] = xpm[i].data[y * xpm[i].width
				+ x];
	}
